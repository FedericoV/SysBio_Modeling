...
def ensemble_log_params(p, params, hess=None,
                        steps=scipy.inf, max_run_hours=scipy.inf,
                        temperature=1.0, step_scale=1.0,
                        sing_val_cutoff=0, seeds=None,
                        recalc_hess_alg = False, recalc_func=None,
    ...
    if scipy.isinf(steps) and scipy.isinf(max_run_hours):
        print 'Both steps and max_run_hours are infinite! Code will not stop by itself!'
...
def _save_ens(ens, ens_Fs, ratio, save_to, attempt_exceptions, steps_attempted,
    ...
    temp_name = save_to + '_temporary_ensemble.pickle'
    if not save_sf:
        pickle.dump((ens, ens_Fs, ratio), temp_name)
    else:
        pickle.dump((ens, ens_Fs, ratio, ens_scale_factors), temp_name)
